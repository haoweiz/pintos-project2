...
start_process (void *file_name_)
{
  ...
  if (!success) {
    thread_current()->load_bool = false;
  ...
  if (kpage != NULL) 
    {
      ...
      if (success){
        *esp = PHYS_BASE;
        char *argument;
        argument = *save_ptr;
        char *token, *save_ptr_two;
        int argc = 1;
        char **address[32];
        char *argv[32];
        argv[0] = (char*)file_name;
        for (token = strtok_r(argument," ",&save_ptr_two); token != NULL;token = strtok_r (NULL, " ", &save_ptr_two),argc++){
          argv[argc] = token;
        }
